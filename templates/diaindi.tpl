...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		{literal}
			$(function()
			{
				$('#datepicker').datepicker().datepicker('option', 'maxDate', '+0d').datepicker('option', 'minDate', {/literal}'{$beginning}'{literal});
				$('#datepicker2').datepicker().datepicker('option', 'maxDate', '+0d').datepicker('option', 'minDate', {/literal}'{$beginning}'{literal});
	...
	<body>
		<div id="diagram">
			<form id="bscForm" class="box" name="bscForm" action="./index.php?page=lightbox&action=dia_indi&id_bsc={bsc->GetIdentifier}&id_pers={pers->GetIdentifier}&id_indi={indi->GetIdentifier}" method="post">
				<fieldset>
					<p>BSC: {bsc->GetName}</p>
					<p>Indikator: {indi->GetName}</p>

					<div class="form-group-box">
						<label for="dia_type">Diagrammtyp</label>
						<select id="dia_type" name="dia_type">
                                                    <option value="Fortschritt">Fortschritt</option>
							<option value="Wertung">Wert</option>
				...
				<fieldset>
					<input value="Anzeigen" type="submit">
				</fieldset>
			</form>
